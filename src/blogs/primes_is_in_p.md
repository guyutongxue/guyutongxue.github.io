...
>
> 以上的讨论是在分圆多项式的数域为有理数域 $\mathbb Q$ 的前提下进行的。（很巧，这些“根”是关于实轴对称（共轭）的，所以它们的乘积也是实数域上的。）而 $n$ 次分圆多项式的次数，就取决于它是多少个“根”乘起来的——或者说 $n$ 次本原单位根有多少个。
> 
> 那么到底有多少个呢？$n$ 次单位根有 $n$ 个是显然的，所以只需要去掉那些是单位根但不是本原单位根的就可以了。考虑第 $i$ 个单位根，它距离 $x$ 正半轴的角度为 $\frac in\cdot2\pi$。但如果 $(i,n)\neq1$ 的话，那么它还可以写成 $\frac {i/(i,n)}{n/(i,n)}\cdot2\pi$。也就是说，这个根同样是 $X^\frac n{(i,n)}-1=0$ 的第 $\frac i{(i,n)}$ 个根，这就不满足本原单位根的定义了。所以，如果第 $i$ 个根是本原单位根，那么它就需要满足 $(i,n)=1$。所以 $n$ 次本原单位根的个数就是比 $n$ 小的、与 $n$ 互素的正整数的个数。这个值恰好就是 $\varphi(n)$。这在抽象代数领域是一个很直观但难以证明的结论：
>> $n$ 次分圆多项式在有理数域上的分裂域的扩张指数为 $\varphi(n)$。
>
> 那么回到现在的问题上，原文中需要构造分圆多项式的数域  $\mathbb F$ 并不是有理数域，而是一个经过模运算后得到的有限域。有限域中，很多结论都是反直觉的，所以也很难形象地将它们讲出来。不过，在之前结论的前提下，通过严谨地运用分裂域、扩张指数的定义和群论中的拉格朗日定理，可以比较简单地证明 $n$ 次分圆多项式在 $\mathbb F_p$ 上的分裂域的扩张指数为 $\mathrm o_r(p)$。
> 
> 如果强行地形象解释这个结论的话，就是说有限域是通过无限域“取模”“得到”的，其中不同的值“转了个圈”可能落到相同的值上。而对于分圆多项式是同样的道理，一个分圆多项式的系数在进行模运算后，它原本一些“与之前不同的（本原的）根”“转到了”“与之前相同（非本原的）的根”（实际上是变得线性相关），而最终剩下的根数恰好为 $\mathrm o_r(p)$。
> 
